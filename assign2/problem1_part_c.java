...
public class problem1_part_c
{
	...
	public static void main(String[] args)
	{
		...
		ArrayList<Packet> finite_sized_queue = new ArrayList<>(size_of_buffer);
		int temp = 1;
        ...
        finite_sized_queue.add(current_packet);
		while(!packetsqueue.isEmpty())
		{
			current_packet = finite_sized_queue.get(0);
            ...
            if(current_packet.Sent(current_packet))
            {
            	if(!packetsqueue.isEmpty())
                {
                	...
                	{
                		Packet extra_packet = finite_sized_queue.get(0);
                        finite_sized_queue.remove(extra_packet);
                	}
                }
		...
		for(int i = 1; i <= n_of_sources; i++)
		{
			try (Writer writer = new FileWriter("problem1_part_b_source"+i+".csv"))
			{
